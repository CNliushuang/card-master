...
plupload.Uploader = function(options) {
    console.log("plupload.Uploader:", options)
	/**
		...
		if (settings.browse_button) {
			plupload.each(settings.browse_button, function(el) {
				queue.push(function(cb) {
				    ...
				    var opt = plupload.extend({}, options, {
                        accept: settings.filters.mime_types,
                        name: settings.file_data_name,
                        multiple: settings.multi_selection,
                        container: settings.container,
                        browse_button: el,
                        webkitdirectory: settings.directory,
                        directory: settings.directory,
                    });
                    console.log('settings:', settings, opt)

					var fileInput = new o.FileInput(opt);
					...
					fileInput.onchange = function() {
                        console.log("报错", settings.max_file_length);
                        if(this.files && this.files.length > settings.max_file_length){
					        var file = this.files[0];
                            self.trigger('Error', {
                                code : plupload.FILE_LENGTH_MAX_ERROR,
                                message : plupload.translate('over max file length limit.')
                            });
